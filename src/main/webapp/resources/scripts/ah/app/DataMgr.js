define( [ "dojo/_base/declare",
            ...
            "dojo/Deferred",
            "dojo/promise/all",
			"ah/util/message/StatusMsg", "dojo/aspect", "dojo/request/notify",
			"ah/config/callbacks","ah/util/dojocover/__AHDialogCache"
		],
    function ( declare, request, lang, array, Deferred, promiseAll, StatusMsg, aspect, notify, callbacks, dialogCache) {
        ...
        var xhrApi = {
				...
				*/
                ...
                return dojo['xhr' + typeMap[type]](lang.mixin({}, {
                    ...
                    error: lang.hitch(this, function (data, response) {
                        var d = data.response;
                        var roleBlocked = data.response.roleBlocked;
                        if(roleBlocked){
                            return ;
                        }
            ...
            _gError: function (url, status, data) {
                var i, len, dialog,
					...
					//popMsg = lang.hitch(PopMsg, PopMsg.show), //registerUI
					error = data.error;

				if( len = dialogCache.length ){
					dialog = dialogCache[len - 1];
					dialog.isFocusable() &&
				    //(showMsg = popMsg = lang.hitch(dialog, dialog.showMsg)); //registerUI
                ...
                switch (status) {
                    ...
                    case 403:
                        ...
                        } else if (error.code == "access.denied.suspended.vhm") {
                            // For suspended VHMs, we redirect to a static page that will eventually trigger logout
                    ...
                    case 401:
                        ...
                        break;
                    // case 403:
                    //     //showMsg('error', i18n.accessDenied);
                    //     break;
                    default:
                        break;
                }
        ...
        notify("send", function (obj, cancel) {
            if(DataMgr.blockedApiList && DataMgr.blockedApiList.length && blockedByRole(DataMgr.blockedApiList, obj)){
                obj.roleBlocked = true;
                cancel();
            }else{
                gFn(obj, 'send');
            }
        ...
        /**
         * To check if current user role can make xhr operations
         */
        function blockedByRole(blockedList, response) {
            var userRole, roleConfig, blockedList, requestUrl, requestMethod;
            var isFailed = false;

            requestUrl = response.url || '';

            isFailed = array.some(blockedList, function(item){
                return requestUrl.indexOf(item) !== -1;
            });

            if(isFailed)console.log('blocked '+ requestUrl);
            return isFailed;
        }
